...
export default function ArticlesPage() {
  ...
  const [publishingTasks, setPublishingTasks] = useState<Record<string, boolean>>({});
  const [selectedImageType, setSelectedImageType] = useState<'with_text' | 'without_text'>('with_text');

  useEffect(() => {
    const fetchArticlesData = async () => {
      try {
        setLoadingProgress({ current: 0, total: 0, step: 'Fetching task IDs...' });
        
        // Step 1: Get all task IDs
        const taskIdsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/content/task_ids`);
        if (!taskIdsResponse.ok) {
          throw new Error(`Failed to fetch task IDs: ${taskIdsResponse.status}`);
        }
        ...
        const taskDataPromises = taskIds.map(async (taskId, index) => {
          try {
            ...
            setLoadingProgress(prev => ({ 
              ...prev, 
              current: index + 1,
              step: `Checking status for task ${index + 1} of ${taskIds.length}...`
            }));
            ...
            if (statusData.status === 'success' || statusData.status === 'published') {
              try {
                // Get image with text
                const imageWithTextResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/content/image/${taskId}?type=image_with_text`);
                ...
                taskData.error = `Image fetch error: ${imgError instanceof Error ? imgError.message : 'Unknown error'}`;
              }
            }
        ...
        const filteredTasks = allTaskData.filter(task => task.status !== 'failed');
    ...
    return () => {
      tasks.forEach(task => {
        ...
        if (task.imageWithoutTextUrl) URL.revokeObjectURL(task.imageWithoutTextUrl);
      });
    };
  }, []);
    ...
    <div className="p-6 w-screen max-w-[94vw] 2xl:max-w-[96vw] overflow-x-auto">
      ...
      {tasks.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-xl font-medium text-gray-500">No articles found</div>
          <div className="text-sm text-gray-400 mt-2">No non-failed tasks were returned from the server</div>
        </div>
      ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {tasks.map((task) => (
                  <tr key={task.task_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      {task.published_image === 'image_with_text' && task.imageWithTextUrl ? (
                      ...
                      ) : task.published_image === 'image_without_text' && task.imageWithoutTextUrl ? (
                        <div className="flex items-center">
                          <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-gray-100">
                            <Image
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        ...
                        {task.status !== 'success' && task.imageWithTextUrl && (
                          <button
                            onClick={() => window.open(task.imageWithTextUrl, '_blank')}
                        ...
                        {task.status === 'published' ? '' : (
                          <button
                            ...
                            disabled={publishingTasks[task.task_id] || task.status === 'published'}
              ...
              {selectedTask.status !== 'published' ? (
                <div className="mb-6">
                  ...
                  <div className="relative w-full h-64 md:h-96 rounded-lg overflow-hidden bg-gray-100">
                    {selectedImageType === 'with_text' && selectedTask.imageWithTextUrl ? (
                      <Image
                      ...
                      />
                    ) : selectedImageType === 'without_text' && selectedTask.imageWithoutTextUrl ? (
                      <Image
                ...
                <div className="relative w-full h-64 md:h-96 rounded-lg overflow-hidden bg-gray-100">
                  {selectedTask.published_image === 'image_with_text' && selectedTask.imageWithTextUrl ? (
                      <Image
                        src={selectedTask.imageWithTextUrl}
                        alt="Image with text"
                        fill
                        className="object-contain"
                      />
                    ) : selectedTask.published_image === 'image_without_text' && selectedTask.imageWithoutTextUrl ? (
                      <Image
                        src={selectedTask.imageWithoutTextUrl}
                        alt="Image without text"
                        fill
